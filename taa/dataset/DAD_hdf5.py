...
from taa.dataset.transforms import VideoTransformTrainHDF5, VideoTransformValHDF5
import time
import matplotlib.pyplot as plt
import math


class DADDatasetHDF5(Dataset):
    """HDF5 기반의 DAD (Driver Accident Dataset) 데이터셋을 위한 PyTorch Dataset 클래스"""
    ...
    def __init__(self, root_path: str, split: str = 'training', transform=None, img_size: Tuple[int, int] = (640, 640)):
        ...
        Args:
            ...
            img_size (tuple): 이미지 크기 (height, width) - HDF5에서는 사용되지 않음
        """
        ...
        # HDF5 파일 경로
        self.hdf5_path = os.path.join(root_path, 'hdf5', f'{split}.hdf5')
        if not os.path.exists(self.hdf5_path):
            raise FileNotFoundError(f"HDF5 파일을 찾을 수 없습니다: {self.hdf5_path}")
        
        # Transform 설정
        self.transform = transform
        if transform is None:
            self.transform = VideoTransformTrainHDF5() if split == 'training' else VideoTransformValHDF5()
        
        # 비디오 리스트 및 annotation 초기화
        self.videos: List[Tuple[str, bool]] = []  # (video_id, is_positive)
        self.annotations: Dict[str, List] = {}
        
        # HDF5 파일에서 비디오 리스트 로드
    ...
    def _load_video_list(self):
        """HDF5 파일에서 positive와 negative 비디오 리스트를 로드"""
        with h5py.File(self.hdf5_path, 'r') as f:
            # Positive 비디오 리스트
            pos_videos = list(f['frames/positive'].keys())
            for video_id in sorted(pos_videos):
            ...
            # Negative 비디오 리스트
            neg_videos = list(f['frames/negative'].keys())
            for video_id in sorted(neg_videos):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        frames = torch.from_numpy(h5_file[f'frames/{category}/{video_id}'][:])  # (T, C, H, W)
        
        # Transform 적용 (이미 텐서 상태이므로 바로 적용)
        frames = self.transform(frames)  # (T, C, H, W)
        
        result = {
            'frames': frames,
            'video_id': video_id,
            'toa': 90. if is_positive else 101.,
...
def create_weighted_sampler(dataset: DADDatasetHDF5) -> WeightedRandomSampler:
...
def collate_fn(batch: list) -> Dict[str, torch.Tensor]:
    ...
    Args:
        batch (list): DADDatasetHDF5.__getitem__()의 반환값 리스트
    ...
    toa = torch.tensor([item['toa'] for item in batch], dtype=torch.float)
    result = {
        'frames': frames,
        'video_ids': video_ids,
        'is_positive': is_positive, 
...
if __name__ == '__main__':
    ...
    def validate_dataset(root_path, split, batch_size=2):
        print(f"\n{'='*20} Validating {split} dataset {'='*20}")
        
        # 1. 데이터 로더 생성 및 기본 정보 출력
        start_time = time.time()
        loader = create_dad_loader_hdf5(
            root_path=root_path,
            split=split,
            batch_size=batch_size,
            num_workers=4
        ...
        plt.savefig(f'taa/dataset/DAD_dataset_hdf5_sample_{split}.png')
        plt.close()
    ...
    # Training set 검증
    validate_dataset(root_path, 'training', batch_size=1)
    
    # Testing set 검증
    validate_dataset(root_path, 'testing', batch_size=1)
